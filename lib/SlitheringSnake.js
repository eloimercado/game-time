  ...
  eatAndGrow(circle1, circle2, canvas, ss) { 
    ...
    case true:
      if (this.orientation === 'right') {
        ss.snake.push(new Block(ss.snake[ss.snake.length - 1].x - 10, ss.snake[ss.snake.length - 1].y))
      } else if (this.orientation === 'left') {
        ss.snake.push(new Block(ss.snake[ss.snake.length - 1].x + 10, ss.snake[ss.snake.length - 1].y))
      } else if (this.orientation === 'up') {
        ss.snake.push(new Block(ss.snake[ss.snake.length - 1].x, ss.snake[ss.snake.length - 1].y + 10))
      } else {
        ss.snake.push(new Block(ss.snake[ss.snake.length - 1].x, ss.snake[ss.snake.length - 1].y - 10))
      }
      return true;
